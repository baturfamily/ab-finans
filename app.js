...
if (data.tarihce && data.tarihce.length > 0) {
  ...
  if (b.doviz && b.doviz !== 'TL' && b.orijinalMiktar > 0) {
    ...
    let gramKur = window.ekraniCizData ? Number(window.ekraniCizData.gramAltinKuru) || window._gramKur || 0 : window._gramKur || 0;
    let guncelKur = b.doviz === 'USD' ? (window.ekraniCizData ? Number(window.ekraniCizData.usdRate) : window._usdKur || 0) : (b.doviz === 'EUR' ? (window.ekraniCizData ? Number(window.ekraniCizData.euroRate) : window._euroKur || 0) : gramKur);
