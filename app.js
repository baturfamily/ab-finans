...
renderTarihceMiniGrafik(data.tarihce || []);
        ...
        try {
            const kartEkstre = data.kartlarDetayli ? data.kartlarDetayli.reduce((t, k) => t + (k.donemIci || 0), 0) : 0;

            let borcTaksit = 0;
            let sabitNakit = 0;
            const _kartIsimleri = window.hesapTurleri ? Object.keys(window.hesapTurleri).filter(k => window.hesapTurleri[k] === 'Kredi Kartı').map(k => k.toLowerCase()) : [];
            ...
            if (data.tumSabitlerListe) {
                data.tumSabitlerListe.forEach(s => {
                    if (s.yon === 'Borç Ödemesi') {
                        ...
                        if (!_kartIsimleri.includes(yontem)) {
                            sabitNakit += s.tutar;
                        }
                    }
                });
