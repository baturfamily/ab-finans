...
renderTarihceMiniGrafik(data.tarihce || []);
        ...
        try {
            ...
            const borcOdemeleri = (toplamGelir - netKalan - toplamSafGider);
            const toplamGiderToplam = toplamSafGider + Math.max(0, borcOdemeleri);
            ...
            if (heroTuketimBar && heroTuketimYuzde) {
                setTimeout(() => {
                    ...
                    if (tuketimYuzde >= 90) {
                        heroTuketimBar.style.background = 'var(--rose)';
                        heroTuketimYuzde.style.color = 'var(--rose)';
                    } else if (tuketimYuzde >= 75) {
                        heroTuketimBar.style.background = 'var(--amber)';
                        heroTuketimYuzde.style.color = 'var(--amber)';
                    } else {
                        heroTuketimBar.style.background = 'var(--emerald)';
                        heroTuketimYuzde.style.color = 'var(--emerald)';
                    }
                    heroTuketimYuzde.innerText = '%' + tuketimYuzde.toFixed(0);
                }, 100);
            }
        } catch(e) {}
...
if (data.tarihce && data.tarihce.length > 0) {
    ...
    for (let i = 0; i < data.tarihce.length; i++) {
        ...
        // Gruplu dropdown: banka hesapları ve kredi kartları ayrı grup başlıklarıyla
        const _bankaHesaplari = [];
        const _krediKartlari = [];
        ...
        if (data.dinamikKategoriler && data.dinamikKategoriler.aktifHesaplar) {
            data.dinamikKategoriler.aktifHesaplar.forEach(hesapAdi => {
                ...
                if (b.tur === "Kredi Kartı") { _krediKartlari.push(optHtml); }
                else { _bankaHesaplari.push(optHtml); }
            });
        } else {
            if (data.bankalar) data.bankalar.forEach(b => {
                window.hesapTurleri[b.isim] = b.tur;
                _bankaHesaplari.push(`<option value="${b.isim}">${b.isim} (${formatTLTam(b.bakiye)})</option>`);
            });
            if (data.kartlarDetayli) data.kartlarDetayli.forEach(k => {
                window.hesapTurleri[k.isim] = "Kredi Kartı";
                _krediKartlari.push(`<option value="${k.isim}">${k.isim} (${formatTLTam(k.borc * -1)})</option>`);
            });
        }

        if (_bankaHesaplari.length > 0) {
            window.hesapOptions += `<option disabled>── 🏦 Banka Hesapları ──</option>` + _bankaHesaplari.join('');
            window.vadesizOptions += `<option disabled>── 🏦 Banka Hesapları ──</option>` + _bankaHesaplari.join('');
        }
        if (_krediKartlari.length > 0) {
            window.hesapOptions += `<option disabled>── 💳 Kredi Kartları ──</option>` + _krediKartlari.join('');
            window.vadesizOptions += `<option disabled>── 💳 Kredi Kartları ──</option>` + _krediKartlari.join('');
        }
