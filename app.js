...
const ekstra = k.donemIci > 0 ? `<div style="font-size:10px;color:var(--text-muted);margin-top:2px;">Dönem içi: <span style="color:var(--rose);font-weight:700;">${formatTL(k.donemIci)}</span> · Gelecek: <span style="color:var(--amber);font-weight:700;">${formatTL(k.gelecek)}</span>${faizSatiri}</div>` : '';
    ...
    <button onclick="openSectionLive(event,'kredi-islemleri-screen','Banka Kredisi İşlemleri'); const m=document.getElementById('action-modal');const bEl=document.getElementById('fab-btn');if(!m.classList.contains('active')){m.classList.add('active');bEl.classList.add('open');document.body.classList.add('modal-open');}" style="height:32px;background:rgba(59,130,246,0.1);border:1px solid rgba(59,130,246,0.2);color:var(--blue);border-radius:8px;font-size:10px;font-weight:800;cursor:pointer;">💳 Kredi Öde</button>
    <button onclick="openSectionLive(event,'kredi-islemleri-screen','Banka Kredisi İşlemleri'); const m=document.getElementById('action-modal');const bEl=document.getElementById('fab-btn');if(!m.classList.contains('active')){m.classList.add('active');bEl.classList.add('open');document.body.classList.add('modal-open');}" style="height:32px;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);color:var(--text-muted);border-radius:8px;font-size:10px;font-weight:800;cursor:pointer;">⚙️ Kredi İşlemleri</button>
</div>`;
...
bankaHtml += `<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-top:10px;">
    <button onclick="openSectionLive(event,'section-hesap-islemleri','Hesap İşlemleri'); const m=document.getElementById('action-modal');const bEl=document.getElementById('fab-btn');if(!m.classList.contains('active')){m.classList.add('active');bEl.classList.add('open');document.body.classList.add('modal-open');} setTimeout(()=>toggleHesapIslemTab('bakiye'),300);" style="height:36px;background:rgba(16,185,129,0.1);border:1px solid rgba(16,185,129,0.2);color:var(--emerald);border-radius:10px;font-size:11px;font-weight:800;cursor:pointer;">💰 Bakiye Güncelle</button>
    <button onclick="const m=document.getElementById('action-modal');const bEl=document.getElementById('fab-btn');if(!m.classList.contains('active')){m.classList.add('active');bEl.classList.add('open');document.body.classList.add('modal-open');} showSection('section-hesap-yonetimi','Banka Hesaplarım');" style="height:36px;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);color:var(--text-muted);border-radius:10px;font-size:11px;font-weight:800;cursor:pointer;">⚙️ Hesap İşlemleri</button>
</div>`;
